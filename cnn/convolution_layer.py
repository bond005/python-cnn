...
class ConvolutionLayer:
    ...
    4) активационная функция каждого нейрона в каждой карте слоя свёртки - это гиперболический
    тангенс;
    5) рецептивное поле слоя свёртки также не обязательно является квадратным, а может быть и
    прямоугольным;
    ...
    def calculate_gradient(self, next_weights, next_gradient, next_receptive_field_size):
        ...
        вычислением проверить соответствие параметров следующего слоя подвыбокри и текущего слоя
