...
class TestConvolutionLayer(unittest.TestCase):
    ...
    def test_weights_test_negative_1(self):
        ...
        new_weights = [[[[random.random() for col_ind in range(self.__receptive_field_size[1])] \
                         for row_ind in range(self.__receptive_field_size[0])] \
                        for inp_ind in range(self.__number_of_input_maps)] \
                       for neuron_ind in range(self.__number_of_feature_maps)]
